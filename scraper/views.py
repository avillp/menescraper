...
class OriginalURLView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.session.get("selected_date"):
            ...
            # Stores the conigured date that is saved in sessions inside the selected_date
        ...
        # That way the SelectDateView can know witch was the last OriginalURL object that the
...
def ChangeDateView(request):
    ...
    # NOTE: All of the sessions created are not cleared so the selected date is permanent
    #            on every OriginalURL object page the user visites, the chart will always show
    #            the configured day
